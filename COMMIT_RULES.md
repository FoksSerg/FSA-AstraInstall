...
- В алгоритме создания коммита используются:
  - `echo`/`sed` для создания `commit_message.txt` (шаг 9)
  - `sed -i` для обновления версий/дат в файлах проекта (шаги 12, 14)
  - `search_replace` или `write` **ТОЛЬКО** для редактирования файла `commit_message.txt` по запросу пользователя (шаг 11, опционально)
- **КРИТИЧНО:** Операции поиска и замены (`search_replace`) можно использовать **ТОЛЬКО** для изменения файла `commit_message.txt` в шаге 11, и больше нигде в алгоритме создания коммита

## 📁 ПРАВИЛО ДЛЯ ИСТОРИИ КОММИТОВ:
**ОБЯЗАТЕЛЬНО ПОСЛЕ КАЖДОГО КОММИТА:** Создавать локальную историю (шаги 22-23). История создается всегда, без дополнительных проверок, так как при каждом коммите меняется версия проекта, что означает изменение исходного кода.

### 🔄 АЛГОРИТМ СОЗДАНИЯ СНИМКА:

**⚠️ ВАЖНО: ТОЧКИ БЕЗОПАСНОГО ПРЕРЫВАНИЯ:**
- **БЕЗОПАСНО прервать БЕЗ изменения файлов проекта:** До шага 12 (включительно шаг 11)
  - Шаги 1-11: только чтение файлов и создание временных файлов (`commit_message.txt`, `.commit_vars.sh`)
  - Переменные сохраняются в `.commit_vars.sh` сразу после определения в шагах 3, 5, 6 (функция `save_vars_to_file()`)
  - Файлы проекта НЕ изменяются
  - Можно безопасно прервать в любой момент
- **ПРЕРЫВАНИЕ С ИЗМЕНЕНИЕМ ФАЙЛОВ (требует отката):** После шага 12, 14 или 15
  - Шаг 12: изменяет даты релиза и версии в измененных файлах проекта (через `sed -i`)
  - Шаг 14: изменяет версии в ключевых файлах проекта (через `sed -i`)
  - Шаг 15: пересобирает бинарные файлы (может занять несколько минут)
   - Если прервать после этих шагов, нужно откатить изменения: `git checkout -- *.py *.sh *.md FSA-AstraInstall-1-7 FSA-AstraInstall-1-8`
- **БЕЗОПАСНО прервать ПЕРЕД СОЗДАНИЕМ КОММИТА:** До шага 19 (включительно шаг 18)
  - Шаг 18: показывает список файлов в индексе, ждет подтверждения пользователя
...
- **ПОСЛЕ КОММИТА:** Шаги 19-25 не изменяют файлы проекта (только git операции и копирование)

1. **Чтение всего алгоритма и вывод количества шагов:**
   - Определить путь к файлу с правилами: `COMMIT_RULES_FILE="COMMIT_RULES.md"`
   - Проверить существование файла: `[ ! -f "$COMMIT_RULES_FILE" ]`
   - **КРИТИЧНО:** Очистить все временные файлы от предыдущих запусков для начала с чистого листа:
     - Удалить `.commit_vars.sh`: `rm -f .commit_vars.sh`
     - Удалить `commit_message.txt`: `rm -f commit_message.txt`
     - Удалить `.commit_analysis_data.txt`: `rm -f .commit_analysis_data.txt`
   - Прочитать весь алгоритм и подсчитать шаги: `grep -E "^# ШАГ [0-9]+:" "$COMMIT_RULES_FILE" | wc -l`
   - Вывести количество найденных шагов и список всех шагов
   - Вывести предупреждение: "⚠️ КРИТИЧНО: Ассистент ОБЯЗАН выполнить ВСЕ N шагов алгоритма"
   - Вывести: "⚠️ ЗАПРЕЩЕНО пропускать шаги или останавливаться раньше времени"
   - Если шаги не найдены - остановиться с ошибкой
...
2. **Проверка директории:** 
   ...
   - Определить текущую дату: `TODAY=$(date +%Y.%m.%d)` и сохранить в `.commit_vars.sh` через `save_vars_to_file()`
...
6. **Определение новой версии:** 
   ...
   - **КРИТИЧНО:** Сохранить переменную `NEW_VERSION` в `.commit_vars.sh` через функцию `save_vars_to_file()` сразу после определения (в шаге 6)
...
8. **Анализ изменений ассистентом:** 
   - Ассистент ОБЯЗАН остановиться после шага 7
   - Прочитать данные из файла `.commit_analysis_data.txt`
   - Проанализировать все изменения в файлах
   - Сформировать краткое понятное описание на русском языке в строго определенном формате (см. шаг 9)
   - Удалить временный файл `.commit_analysis_data.txt` после чтения
   - Пользователь НЕ видит процесс анализа

9. **Сохранение описания:** 
   - Загрузить `TODAY` из `.commit_vars.sh` (определена в шаге 2): `source .commit_vars.sh`
   - Если `TODAY` не загружена - определить: `TODAY=$(date +%Y.%m.%d)`
   - Формат описания СТРОГО определен:
     ...
     - (указывать ТОЛЬКО если были удалены классы/модули, иначе эту секцию не включать)
     
     Проект: FSA-AstraInstall
     Версия: $NEW_VERSION ($TODAY)
     ```
   - **КРИТИЧНО: НЕ комментировать изменения дат и версий в файлах проекта** - эти изменения делаются автоматически алгоритмом (шаги 12 и 14) и не должны упоминаться в описании коммита
   - В описании указывать ТОЛЬКО реальные изменения кода, логики, правил, структуры и т.д.
   - **КРИТИЧНО:** Секцию "Описание удаленных классов/модулей" включать ТОЛЬКО если были удалены классы/модули, иначе эту секцию не включать в описание
   - Сохранить описание в файл `commit_message.txt` ТОЛЬКО через `echo`/`sed` (ЗАПРЕЩЕНО использовать `write` tool)
   - В строке "Версия:" использовать формат `$NEW_VERSION ($TODAY)` (переменные NEW_VERSION из шага 6 и TODAY из шага 9)
   - Версия уже содержит дату в скобках, отдельную строку с датой не указывать
   - Пример команды: `echo "[Заголовок]" > commit_message.txt && echo "" >> commit_message.txt && ... && echo "Версия: $NEW_VERSION ($TODAY)" >> commit_message.txt`
   - ТОЛЬКО после сохранения файла продолжать выполнение шага 10

10. **Проверка файла сообщения:** 
...
11. **ОБЯЗАТЕЛЬНАЯ ПАУЗА:** 
   - Вывести: "=== Описание коммита готово ==="
   - **КРИТИЧНО:** Вывести содержимое файла `commit_message.txt` через `cat commit_message.txt` (пользователь ДОЛЖЕН видеть полное содержимое файла)
   - Вывести разделитель до и после содержимого для наглядности
   - **КРИТИЧНО:** Переменные уже сохранены в файл `.commit_vars.sh` в шагах 3, 5, 6 (функция `save_vars_to_file()`)
   ...
   - ТОЛЬКО после получения ответа пользователя в чате ассистент продолжает выполнение шага 12
   - **⚠️ ПОСЛЕДНЯЯ ТОЧКА БЕЗОПАСНОГО ПРЕРЫВАНИЯ БЕЗ ИЗМЕНЕНИЯ ФАЙЛОВ ПРОЕКТА**

12. **Обновление дат релиза и версий в измененных и новых файлах:** 
   ...
   - Загрузить `TODAY` из `.commit_vars.sh` (определена в шаге 2), если не загружена - определить: `TODAY=$(date +%Y.%m.%d)`
   - Объединить измененные и новые файлы: `ALL_FILES_TO_UPDATE="$CHANGED_FILES $NEW_FILES"`
   - Для каждого файла из `ALL_FILES_TO_UPDATE` (использовать `echo "$ALL_FILES_TO_UPDATE" | tr ' ' '\n' | while read file` для правильного разделения строки), если файл имеет расширение `.py`, `.sh` или `.md`:
...
13. **Проверка обновления дат:** 
   - Загрузить `TODAY` из `.commit_vars.sh`: `source .commit_vars.sh` (если не загружена)
   - Объединить измененные и новые файлы: `ALL_FILES_TO_CHECK="$CHANGED_FILES $NEW_FILES"`
   - Для каждого файла из `ALL_FILES_TO_CHECK` (использовать `echo "$ALL_FILES_TO_CHECK" | tr ' ' '\n' | while read file` для правильного разделения строки), если файл имеет расширение `.py`, `.sh` или `.md`:
...
14. **Обновление версий проекта в ключевых файлах и новых файлах:** 
   ...
   - **КРИТИЧНО:** Для каждого файла из `NEW_FILES` (использовать `echo "$NEW_FILES" | tr ' ' '\n' | while read file`), если файл имеет расширение `.py`, `.sh` или `.md` и содержит версию в первых 20 строках:
     - Проверить существование файла: `[ -f "$file" ]`
     - Проверить наличие версии: `head -20 "$file" | grep -q "V[0-9][0-9]*\\.[0-9][0-9]*\\.[0-9][0-9]*"`
     - Если версия найдена: обновить версию на `NEW_VERSION` через `sed -i '' "1,20s/V[0-9][0-9]*\\.[0-9][0-9]*\\.[0-9][0-9]*/${NEW_VERSION}/g" "$file"`
     - Вывести: "✓ Версия обновлена в новом файле $file"
   - **⚠️ ИЗМЕНЯЕТ ФАЙЛЫ ПРОЕКТА**

15. **Пересборка бинарных файлов:** 
...
18. **ОБЯЗАТЕЛЬНАЯ ПАУЗА ПЕРЕД СОЗДАНИЕМ КОММИТА:** 
   ...
   - **МЕХАНИЗМ РАЗРЕШЕНИЯ:** Пользователь отвечает в чате одним из способов:
     ...
     - Любой другой ответ - ассистент интерпретирует как разрешение или уточняет намерение
   - ТОЛЬКО после получения явного подтверждения пользователя в чате ассистент продолжает выполнение шага 19
   - **⚠️ ПОСЛЕДНЯЯ ТОЧКА БЕЗОПАСНОГО ПРЕРЫВАНИЯ ПЕРЕД СОЗДАНИЕМ КОММИТА**
...
22. **Создание локальной истории:**
   ...
   - Вывести: "✓ Файлы скопированы в History/$COMMIT_DATE/"

23. **Валидация истории:**
   - Выполнить `ls -la "History/$COMMIT_DATE/"` для проверки
   - Если файлы не скопированы - остановиться с ошибкой
   - Вывести: "✓ Файлы истории проверены"

24. **Подтверждение завершения и отчёт:**
...
25. **ФИНАЛЬНАЯ ПРОВЕРКА ВЫПОЛНЕНИЯ ВСЕХ ШАГОВ:**
   - Вывести: "=== ШАГ 25: Финальная проверка выполнения всех шагов ==="
   - Вывести список всех обязательных шагов (1-24) с отметками выполнения
   - Проверить, что все шаги выполнены успешно
   - Если какой-то шаг не выполнен - остановиться с ошибкой
   - Вывести: "✓ Все шаги (1-24) выполнены успешно"
   - Вывести: "=== Процесс создания снимка полностью завершён ==="
...
- **ЗАПРЕЩЕНО ПРОПУСКАТЬ ШАГИ** - все шаги с 1 по 25 должны быть выполнены строго по порядку
...
- **ВАЖНО:** Переменные сохраняются в файл `.commit_vars.sh` сразу после определения в шагах 3, 5, 6 (функция `save_vars_to_file()`). Перед продолжением после паузы ассистент ДОЛЖЕН загрузить их: `source .commit_vars.sh` или `. .commit_vars.sh`

```bash
# КРИТИЧНО: Функции должны быть определены ПЕРЕД использованием
# Функция для остановки процесса при ошибках
...
save_vars_to_file() {
    ...
    [ ! -z "$TODAY" ] && echo 'TODAY="'$TODAY'"' >> "$VARS_FILE" || echo 'TODAY=""' >> "$VARS_FILE"
}

# ШАГ 1: ЧТЕНИЕ ВСЕГО АЛГОРИТМА И ВЫВОД КОЛИЧЕСТВА ШАГОВ
echo "=== ШАГ 1: Чтение всего алгоритма создания снимка ==="
echo ""
echo "📖 Ассистент читает весь алгоритм до конца..."
echo ""

# Определяем путь к файлу с правилами
COMMIT_RULES_FILE="COMMIT_RULES.md"
[ ! -f "$COMMIT_RULES_FILE" ] && stop_on_error "Файл $COMMIT_RULES_FILE не найден"

# КРИТИЧНО: Очищаем все временные файлы от предыдущих запусков для начала с чистого листа
echo "Очистка временных файлов от предыдущих запусков..."
rm -f .commit_vars.sh commit_message.txt .commit_analysis_data.txt
echo "✓ Временные файлы очищены"

# Читаем весь алгоритм и подсчитываем шаги
TOTAL_STEPS=$(grep -E "^# ШАГ [0-9]+:" "$COMMIT_RULES_FILE" | wc -l | xargs)
# xargs убирает пробелы

# Проверяем, что шаги найдены
[ -z "$TOTAL_STEPS" ] || [ "$TOTAL_STEPS" -eq 0 ] && stop_on_error "Не удалось определить количество шагов в алгоритме"

# Выводим информацию о шагах
echo "Найдено шагов в алгоритме: $TOTAL_STEPS"
echo ""
echo "Список всех шагов алгоритма:"
echo "----------------------------------------"
grep -E "^# ШАГ [0-9]+:" "$COMMIT_RULES_FILE" | sed 's/^# ШАГ /  Шаг /' | sed 's/:$//'
echo "----------------------------------------"
echo ""
echo "⚠️ КРИТИЧНО: Ассистент ОБЯЗАН выполнить ВСЕ $TOTAL_STEPS шагов алгоритма"
echo "⚠️ ЗАПРЕЩЕНО пропускать шаги или останавливаться раньше времени"
echo ""
echo "✓ Алгоритм прочитан. Начинаем выполнение с шага 2."
echo ""
...
# ШАГ 2: КРИТИЧНО: Проверка и переход в директорию проекта
echo "=== ШАГ 2: Проверка директории ==="
...
# КРИТИЧНО: Определяем TODAY один раз в начале и сохраняем
TODAY=$(date +%Y.%m.%d)
save_vars_to_file
echo "✓ Дата определена: $TODAY (сохранена в .commit_vars.sh)"
...
# ШАГ 5: Определяем текущую версию в измененных и новых файлах
echo "=== ШАГ 5: Определение текущей версии ==="
...
# КРИТИЧНО: Сохраняем переменные сразу после определения
save_vars_to_file
echo "✓ Переменные шага 5 сохранены в .commit_vars.sh"

# ШАГ 6: Определяем новую версию проекта
echo "=== ШАГ 6: Определение новой версии ==="
...
# КРИТИЧНО: Проверка успешности шага 6
STEP6_EXIT_CODE=$?
if [ $STEP6_EXIT_CODE -ne 0 ]; then
    stop_on_error "ШАГ 6 не выполнен успешно"
fi
...
# КРИТИЧНО: Сохраняем переменные сразу после определения
save_vars_to_file
echo "✓ Переменные шага 6 сохранены в .commit_vars.sh"

# ШАГ 7: Собираем подробную информацию об изменениях и сохраняем в файл
echo "=== ШАГ 7: Сбор подробной информации об изменениях ==="
echo "=== Сбор данных для анализа... ==="
# Загружаем TODAY из .commit_vars.sh (определена в шаге 2)
[ -f ".commit_vars.sh" ] && source .commit_vars.sh || true
...
echo ""
echo "=========================================="
# КРИТИЧНО: Проверка успешности шага 7
STEP7_EXIT_CODE=$?
if [ $STEP7_EXIT_CODE -ne 0 ]; then
    stop_on_error "ШАГ 7 не выполнен успешно"
fi

# ШАГ 8: Анализ изменений ассистентом
# КРИТИЧНО: После выполнения ШАГ 7 (когда команда завершилась и выведены маркеры паузы) ассистент должен:
...
# 8. ТОЛЬКО после сохранения файла продолжать выполнение следующей команды (ШАГ 10)
# Пользователь НЕ видит процесс анализа
#
...
#
# Проект: FSA-AstraInstall
# Версия: $NEW_VERSION ($TODAY) (использовать переменные NEW_VERSION из шага 6 и TODAY из шага 9)
#
# ПРИМЕР команды для сохранения (ассистент должен выполнить после анализа):
# TODAY=$(date +%Y.%m.%d)
# # NEW_VERSION уже определена в шаге 6
...
# ... и т.д.
# # Секцию "Описание удаленных классов/модулей" включать ТОЛЬКО если были удалены классы/модули
# echo "Проект: FSA-AstraInstall" >> commit_message.txt
# echo "Версия: $NEW_VERSION ($TODAY)" >> commit_message.txt
#
# После сохранения ассистент должен сообщить что описание готово и можно продолжать

# ШАГ 9: Сохранение описания
# КРИТИЧНО: Ассистент должен сохранить описание в файл commit_message.txt через echo/sed
# Формат описания определен в текстовом описании шага 9

# ШАГ 10: Проверяем файл сообщения коммита
echo "=== ШАГ 10: Проверка файла сообщения коммита ==="
[ ! -f "commit_message.txt" ] || [ ! -s "commit_message.txt" ] && stop_on_error "Файл commit_message.txt не найден или пуст. Ассистент должен был сформировать описание в ШАГ 8-9." || true
...
# ШАГ 11: Ожидание разрешения на продолжение (возможность редактирования файла)
echo "=== ШАГ 11: Ожидание разрешения на продолжение ==="
echo ""
...
if [ ! -f "$VARS_FILE" ] || [ ! -s "$VARS_FILE" ]; then
    stop_on_error "Файл $VARS_FILE не был создан или пуст. Переменные должны были быть сохранены в шагах 3, 5, 6."
fi
...
echo "6. ТОЛЬКО после получения ответа пользователя в чате продолжайте выполнение следующих команд (шаг 12)"
echo ""
echo "⚠️ ВАЖНО: Перед продолжением (шаг 12) ассистент ДОЛЖЕН загрузить переменные из файла .commit_vars.sh:"
echo "   source .commit_vars.sh"
echo "   или"
echo "   . .commit_vars.sh"
echo ""
echo "❌ ЗАПРЕЩЕНО: Продолжать выполнение автоматически без ответа пользователя!"
echo "❌ ЗАПРЕЩЕНО: Выполнять следующие команды (шаг 12 и далее) без явного разрешения!"
...
# ШАГ 12: Обновляем дату релиза и версии в измененных и новых файлах
echo "=== ШАГ 12: Загрузка переменных и проверка файла ==="
...
# Загружаем TODAY из .commit_vars.sh (определена в шаге 2)
[ -z "$TODAY" ] && TODAY=$(date +%Y.%m.%d) && save_vars_to_file
echo "✓ Дата для обновления: $TODAY"
echo "=== ШАГ 12: Обновление дат релиза и версий в измененных и новых файлах ==="
[ "$(basename $(pwd))" != "FSA-AstraInstall" ] && cd /Volumes/FSA-PRJ/Project/FSA-AstraInstall || true
# Объединяем измененные и новые файлы
...
# ШАГ 13: Проверяем обновление дат релиза
echo "=== ШАГ 13: Проверка обновления дат релиза ==="
# Загружаем TODAY из .commit_vars.sh (определена в шаге 2)
[ -z "$TODAY" ] && source .commit_vars.sh || true
...
# ШАГ 14: Обновляем версию проекта в ключевых файлах и новых файлах
echo "=== ШАГ 14: Обновление версий проекта в ключевых файлах и новых файлах ==="
# КРИТИЧНО: Проверка директории проекта
PROJECT_DIR="/Volumes/FSA-PRJ/Project/FSA-AstraInstall"
[ "$(basename $(pwd))" != "FSA-AstraInstall" ] && cd "$PROJECT_DIR" || true
[ "$(basename $(pwd))" != "FSA-AstraInstall" ] && stop_on_error "Не удалось перейти в директорию проекта" || true
...
if [ ! -f ".commit_vars.sh" ]; then
    stop_on_error "Файл .commit_vars.sh не найден. Невозможно продолжить без переменных."
fi
source .commit_vars.sh || stop_on_error "Не удалось загрузить переменные из .commit_vars.sh"
...
# КРИТИЧНО: Обновляем версии в новых файлах
echo "$NEW_FILES" | tr ' ' '\n' | while read file; do
    [ ! -z "$file" ] || continue
    if [[ "$file" =~ \.(py|sh|md)$ ]] && [ -f "$file" ]; then
        # Проверяем наличие версии в первых 20 строках
        if head -20 "$file" | grep -q "V[0-9][0-9]*\\.[0-9][0-9]*\\.[0-9][0-9]*"; then
            echo "Обработка нового файла: $file"
            [ -z "$NEW_VERSION" ] && echo "  ✗ ОШИБКА: NEW_VERSION пуста, пропускаем $file" || \
            sed -i '' "1,20s/V[0-9][0-9]*\\.[0-9][0-9]*\\.[0-9][0-9]*/${NEW_VERSION}/g" "$file" 2>/dev/null || true
            echo "  ✓ Версия обновлена в новом файле $file"
        fi
    fi
done
echo "✓ Версии обновлены в ключевых файлах и новых файлах"
...
# ШАГ 15: Пересборка бинарных файлов
echo "=== ШАГ 15: Пересборка бинарных файлов ==="
# КРИТИЧНО: Проверка директории проекта
PROJECT_DIR="/Volumes/FSA-PRJ/Project/FSA-AstraInstall"
[ "$(basename $(pwd))" != "FSA-AstraInstall" ] && cd "$PROJECT_DIR" || true
[ "$(basename $(pwd))" != "FSA-AstraInstall" ] && stop_on_error "Не удалось перейти в директорию проекта" || true
...
# ШАГ 16: Проверяем обновление версий в ключевых файлах
echo "=== ШАГ 16: Проверка обновления версий ==="
# КРИТИЧНО: Проверка директории проекта
PROJECT_DIR="/Volumes/FSA-PRJ/Project/FSA-AstraInstall"
[ "$(basename $(pwd))" != "FSA-AstraInstall" ] && cd "$PROJECT_DIR" || true
[ "$(basename $(pwd))" != "FSA-AstraInstall" ] && stop_on_error "Не удалось перейти в директорию проекта" || true
...
# ШАГ 17: Добавляем измененные файлы явно (НЕ используем git add .)
echo "=== ШАГ 17: Добавление файлов в индекс ==="
# КРИТИЧНО: Проверка директории проекта
PROJECT_DIR="/Volumes/FSA-PRJ/Project/FSA-AstraInstall"
[ "$(basename $(pwd))" != "FSA-AstraInstall" ] && cd "$PROJECT_DIR" || true
[ "$(basename $(pwd))" != "FSA-AstraInstall" ] && stop_on_error "Не удалось перейти в директорию проекта" || true
...
# ШАГ 18: ОБЯЗАТЕЛЬНАЯ ПАУЗА ПЕРЕД СОЗДАНИЕМ КОММИТА
echo "=== ШАГ 18: Ожидание подтверждения перед созданием коммита ==="
echo ""
...
echo "6. ТОЛЬКО после получения явного подтверждения пользователя в чате продолжайте выполнение следующих команд (шаг 19)"
echo ""
echo "❌ ЗАПРЕЩЕНО: Продолжать выполнение автоматически без ответа пользователя!"
echo "❌ ЗАПРЕЩЕНО: Выполнять следующие команды (шаг 19 и далее) без явного разрешения!"
echo ""
echo "=========================================="
...
# ШАГ 19: Создаем коммит
echo "=== ШАГ 19: Создание коммита ==="
...
STEP19_EXIT_CODE=$?
if [ $STEP19_EXIT_CODE -ne 0 ]; then
    stop_on_error "ШАГ 19 не выполнен успешно"
fi

# ШАГ 20: Верификация коммита
echo "=== ШАГ 20: Верификация коммита ==="
...
# ШАГ 21: Удаляем временные файлы
echo "=== ШАГ 21: Удаление временных файлов ==="
rm -f commit_message.txt .commit_vars.sh .commit_analysis_data.txt
echo "✓ Временные файлы удалены"
# КРИТИЧНО: Проверка успешности шага 21
if [ $? -ne 0 ]; then
    stop_on_error "ШАГ 21 не выполнен успешно"
fi

# ШАГ 22: Создаем локальную историю
echo "=== ШАГ 22: Создание локальной истории ==="
...
echo "✓ Файлы скопированы в History/$COMMIT_DATE/"
# КРИТИЧНО: Проверка успешности шага 22
if [ $? -ne 0 ]; then
    stop_on_error "ШАГ 22 не выполнен успешно"
fi

# ШАГ 23: Валидация истории
echo "=== ШАГ 23: Валидация истории ==="
...
echo "✓ Файлы истории проверены"
# КРИТИЧНО: Проверка успешности шага 23
if [ $? -ne 0 ]; then
    stop_on_error "ШАГ 23 не выполнен успешно"
fi

# ШАГ 24: Подтверждение завершения и отчёт
echo "=== ШАГ 24: Подтверждение завершения и отчёт ==="
...
# КРИТИЧНО: Проверка успешности шага 24
if [ $? -ne 0 ]; then
    stop_on_error "ШАГ 24 не выполнен успешно"
fi

# ШАГ 25: ФИНАЛЬНАЯ ПРОВЕРКА ВЫПОЛНЕНИЯ ВСЕХ ШАГОВ
echo "=== ШАГ 25: Финальная проверка выполнения всех шагов ==="
echo ""
echo "Проверка выполнения обязательных шагов (1-24):"
echo "----------------------------------------"

# Реальные проверки выполнения шагов
VERIFICATION_ERRORS=0

# Проверка шага 1: файл COMMIT_RULES.md должен существовать
[ -f "COMMIT_RULES.md" ] && echo "✓ Шаг 1: Чтение всего алгоритма (файл COMMIT_RULES.md найден)" || (echo "✗ Шаг 1: Файл COMMIT_RULES.md не найден" && VERIFICATION_ERRORS=$((VERIFICATION_ERRORS + 1)))

# Проверка шага 2: директория должна быть правильной
[ "$(basename $(pwd))" = "FSA-AstraInstall" ] && echo "✓ Шаг 2: Проверка директории (текущая директория: $(pwd))" || (echo "✗ Шаг 2: Неправильная директория: $(pwd)" && VERIFICATION_ERRORS=$((VERIFICATION_ERRORS + 1)))

# Проверка шага 3-6: переменные должны быть сохранены (если файл еще не удален)
[ -f ".commit_vars.sh" ] && echo "✓ Шаг 3-6: Переменные сохранены (файл .commit_vars.sh существует)" || echo "ℹ Шаг 3-6: Файл .commit_vars.sh удален (это нормально после шага 21)"

# Проверка шага 7-9: файл commit_message.txt должен быть удален (после шага 21)
[ ! -f "commit_message.txt" ] && echo "✓ Шаг 7-9: Файл commit_message.txt удален (это нормально)" || (echo "✗ Шаг 7-9: Файл commit_message.txt все еще существует" && VERIFICATION_ERRORS=$((VERIFICATION_ERRORS + 1)))

# Проверка шага 10: commit_message.txt должен был существовать (проверяется в шаге 10)
echo "✓ Шаг 10: Проверка файла сообщения (выполнен в шаге 10)"

# Проверка шага 11: пауза была выполнена
echo "✓ Шаг 11: ОБЯЗАТЕЛЬНАЯ ПАУЗА (выполнена)"

# Проверка шага 12-13: даты должны быть обновлены в измененных файлах
# (проверяется в шаге 13, здесь просто подтверждаем)
echo "✓ Шаг 12-13: Обновление и проверка дат релиза (выполнено в шагах 12-13)"

# Проверка шага 14-16: версии должны быть обновлены в ключевых файлах
KEY_FILES="FSA-AstraInstall.py README.md Version.txt DocInstruction/WINE_INSTALL_GUIDE.md"
VERSION_CHECK_OK=1
for file in $KEY_FILES; do
    [ -f "$file" ] || continue
    if [ "$file" = "Version.txt" ]; then
        grep -q "^APP_VERSION=" "$file" || VERSION_CHECK_OK=0
    else
        head -20 "$file" | grep -q "V[0-9][0-9]*\\.[0-9][0-9]*\\.[0-9][0-9]*" || VERSION_CHECK_OK=0
    fi
done
[ $VERSION_CHECK_OK -eq 1 ] && echo "✓ Шаг 14-16: Обновление и проверка версий (версии найдены в ключевых файлах)" || (echo "✗ Шаг 14-16: Версии не найдены в ключевых файлах" && VERIFICATION_ERRORS=$((VERIFICATION_ERRORS + 1)))

# Проверка шага 15: бинарные файлы должны существовать
[ -f "FSA-AstraInstall-1-7" ] && [ -f "FSA-AstraInstall-1-8" ] && echo "✓ Шаг 15: Пересборка бинарных файлов (файлы существуют)" || (echo "✗ Шаг 15: Бинарные файлы не найдены" && VERIFICATION_ERRORS=$((VERIFICATION_ERRORS + 1)))

# Проверка шага 17: файлы должны быть в индексе (проверяется в шаге 17)
echo "✓ Шаг 17: Добавление файлов в индекс (выполнено в шаге 17)"

# Проверка шага 18: пауза была выполнена
echo "✓ Шаг 18: ОБЯЗАТЕЛЬНАЯ ПАУЗА ПЕРЕД КОММИТОМ (выполнена)"

# Проверка шага 19-20: коммит должен быть создан
COMMIT_HASH=$(git log -1 --format="%H" 2>/dev/null)
[ ! -z "$COMMIT_HASH" ] && echo "✓ Шаг 19-20: Создание и верификация коммита (коммит найден: ${COMMIT_HASH:0:8}...)" || (echo "✗ Шаг 19-20: Коммит не найден" && VERIFICATION_ERRORS=$((VERIFICATION_ERRORS + 1)))

# Проверка шага 21: временные файлы должны быть удалены
[ ! -f ".commit_vars.sh" ] && [ ! -f ".commit_analysis_data.txt" ] && echo "✓ Шаг 21: Удаление временных файлов (файлы удалены)" || (echo "✗ Шаг 21: Временные файлы все еще существуют" && VERIFICATION_ERRORS=$((VERIFICATION_ERRORS + 1)))

# Проверка шага 22-23: история должна быть создана
[ ! -z "$COMMIT_DATE" ] || COMMIT_DATE=$(date +%Y_%m_%d_%H_%M)
[ -d "History/$COMMIT_DATE" ] && [ -f "History/$COMMIT_DATE/FSA-AstraInstall.py" ] && echo "✓ Шаг 22-23: Создание и валидация истории (директория History/$COMMIT_DATE существует)" || (echo "✗ Шаг 22-23: История не создана или неполная" && VERIFICATION_ERRORS=$((VERIFICATION_ERRORS + 1)))

# Проверка шага 24: отчёт был выведен
echo "✓ Шаг 24: Подтверждение завершения и отчёт (выполнен в шаге 24)"

echo "----------------------------------------"
if [ $VERIFICATION_ERRORS -eq 0 ]; then
    echo "✓ Все шаги (1-24) выполнены успешно"
    echo ""
    echo "=== Процесс создания снимка полностью завершён ==="
else
    echo "✗ Обнаружено ошибок при проверке: $VERIFICATION_ERRORS"
    stop_on_error "ШАГ 25: Обнаружены ошибки при проверке выполнения шагов"
fi
# КРИТИЧНО: Проверка успешности шага 25
STEP25_EXIT_CODE=$?
if [ $STEP25_EXIT_CODE -ne 0 ]; then
    stop_on_error "ШАГ 25 не выполнен успешно"
fi
```
