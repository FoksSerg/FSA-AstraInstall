...
def build_in_docker():
    ...
    # ВАЖНО: Используем родной Python 3.7 из Debian Buster (GLIBC 2.28)
    # Это ГАРАНТИРУЕТ работу на: Astra 1.7.x, 1.8.x, Ubuntu 20.04+, Debian 10+
    dockerfile_content = '''FROM debian:buster

# Архивные репозитории Debian Buster (EOL)
RUN echo 'deb http://archive.debian.org/debian buster main' > /etc/apt/sources.list && \\
    echo 'deb http://archive.debian.org/debian-security buster/updates main' >> /etc/apt/sources.list && \\
    echo 'Acquire::Check-Valid-Until "false";' > /etc/apt/apt.conf.d/99no-check-valid-until

# Устанавливаем Python 3.7 (родной для Buster) и GUI зависимости
RUN apt-get update && apt-get install -y \\
    python3 python3-pip python3-dev \\
    python3-tk tk-dev tcl-dev \\
    libx11-dev libxft-dev gcc \\
    && rm -rf /var/lib/apt/lists/*

# Устанавливаем PyInstaller и psutil
RUN pip3 install pyinstaller psutil
...
'''
    ...
    # .dockerignore (исключаем большие папки для ускорения передачи)
...
'''
    ...
    # Создаём .dockerignore (без .build) чтобы Docker его увидел
...
echo "[#] Компиляция FSA-AstraInstall.py в FSA-AstraInstall..."
...
def cleanup():
    ...
    # Удаляем старый .dockerignore.build если остался
    old_dockerignore = SCRIPT_DIR / ".dockerignore.build"
    if old_dockerignore.exists():
        old_dockerignore.unlink()
