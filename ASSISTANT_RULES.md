...
# Версия проекта: V2.6.132 (2025.11.24)
...
1. **ПРОВЕРКА СИНТАКСИСА ПОСЛЕ КАЖДОГО ИЗМЕНЕНИЯ:**
   - ✅ **ВСЕГДА** после каждого `search_replace` или редактирования проверять синтаксис: `python3 -m py_compile <файл>`
...
2. **МИНИМАЛЬНЫЙ КОНТЕКСТ ДЛЯ ЗАМЕНЫ:**
   - ✅ Использовать минимально необходимый контекст в `search_replace` (не более 5-7 строк уникального кода)
   - ✅ Включать только уникальные строки, которые точно идентифицируют место замены
   - ❌ **ЗАПРЕЩЕНО** заменять большие блоки кода (>20 строк) за один раз

3. **ТОЧЕЧНЫЕ ИЗМЕНЕНИЯ:**
   - ✅ Вносить изменения **ПО ОДНОМУ МЕСТУ** за раз
   - ❌ **ЗАПРЕЩЕНО** применять массовые автоматические исправления отступов через скрипты
   - ✅ Если нужно исправить несколько мест - делать это **ПОСЛЕДОВАТЕЛЬНО** с проверкой после каждого
