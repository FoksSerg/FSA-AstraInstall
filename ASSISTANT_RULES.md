...
1. **Получить количество коммитов:**
   ```bash
   COMMIT_COUNT=$(git rev-list --count HEAD)
   NEW_VERSION="V2.2.$COMMIT_COUNT"
   ```

2. **Обновить версию проекта во всех файлах:**
   ```bash
   # Обновляем версию проекта во всех файлах
   sed -i "s/V2\.2\.[0-9]*/V2.2.$COMMIT_COUNT/g" astra_automation.py
   sed -i "s/V2\.2\.[0-9]*/V2.2.$COMMIT_COUNT/g" astra_install.sh
   sed -i "s/V2\.2\.[0-9]*/V2.2.$COMMIT_COUNT/g" astra_update.sh
   sed -i "s/V2\.2\.[0-9]*/V2.2.$COMMIT_COUNT/g" astra_update.py
   sed -i "s/V2\.2\.[0-9]*/V2.2.$COMMIT_COUNT/g" README.md
   sed -i "s/V2\.2\.[0-9]*/V2.2.$COMMIT_COUNT/g" UPDATE_SCRIPTS_README.md
   ```

3. **Обновить дату релиза в измененных файлах:**
   ```bash
   TODAY=$(date +%Y.%m.%d)
   # Обновляем дату только в файлах, которые были изменены в текущей сессии
   ```
...
**Результат:** Все файлы попадают в коммит с актуальными версиями проекта + создается локальная история!

## 🏷️ ПРАВИЛА ВЕРСИОНИРОВАНИЯ:

### Схема версий:
- **Версия проекта**: `V2.2.X` (MAJOR.MINOR.КОЛИЧЕСТВО_КОММИТОВ) - общая для всех файлов проекта
- **Дата релиза файла**: `YYYY.MM.DD` - индивидуальная для каждого файла

### Формат в файлах:
```bash
# Версия проекта: V2.2.X
...
3. **MAJOR версия** (V2.X.X) - кардинальные изменения архитектуры
4. **MINOR версия** (V2.2.X) - новые функции, улучшения
5. **PATCH версия** (V2.2.X) - количество коммитов (сохраненных изменений)

### Примеры:
```
V2.2.58 (2025.10.15) - текущая версия проекта (58 коммитов)
├── astra_automation.py: V2.2.58 (2025.10.15) ← изменен сегодня
├── astra_install.sh: V2.2.58 (2025.10.14) ← изменен вчера
├── astra_update.sh: V2.2.58 (2025.10.13) ← изменен позавчера
└── README.md: V2.2.58 (2025.10.15) ← обновлен сегодня
```
