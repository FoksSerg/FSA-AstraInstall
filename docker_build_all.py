...
# 1. Если скрипт в корне проекта - используем текущую директорию
# 2. Если скрипт в RunScript/ - используем родительскую директорию
# 3. Проверяем наличие DockerManager для определения правильного пути

# Сначала проверяем, есть ли DockerManager в текущей директории скрипта
if (SCRIPT_DIR / "DockerManager").exists():
    PROJECT_DIR = SCRIPT_DIR
elif (SCRIPT_DIR.parent / "DockerManager").exists():
    PROJECT_DIR = SCRIPT_DIR.parent
else:
    # Если не нашли, пробуем найти DockerManager в текущей рабочей директории
    current_dir = Path.cwd()
    if (current_dir / "DockerManager").exists():
        PROJECT_DIR = current_dir
    else:
        print(f"❌ ОШИБКА: Модуль DockerManager не найден")
        print(f"Проверенные пути:")
        print(f"  - {SCRIPT_DIR / 'DockerManager'}")
        print(f"  - {SCRIPT_DIR.parent / 'DockerManager'}")
        print(f"  - {current_dir / 'DockerManager'}")
        print(f"Убедитесь, что скрипт запускается из директории проекта FSA-AstraInstall")
        sys.exit(1)

# Проверяем, что DockerManager действительно существует
