...
def run_gui_monitor(temp_dir, dry_run=False):
    ...
    if not os.path.exists(gui_monitor_path):
        print("❌ Модуль gui_monitor.py не найден")
        return False
    ...
    try:
        ...
        if result == 0:
            print("✅ GUI мониторинг завершен успешно!")
            return True
        else:
            print("❌ Ошибка GUI мониторинга (код: %d)" % result)
            return False
...
def main():
    ...
    try:
        ...
        # Если запрошен GUI режим, запускаем GUI
        if gui_mode:
            print("🖥️ Запуск GUI мониторинга...")
            gui_success = run_gui_monitor(temp_dir, dry_run)
            if gui_success:
                print("✅ GUI мониторинг завершен")
            else:
                print("❌ Ошибка GUI мониторинга")
            return
