# FSA-AstraInstall Automation V3.0.146 (2025.12.03)
...
**Текущая версия:** V3.0.146 (коммит #145)  
...
├── build_executables.py         # Скрипт сборки бинарника
...
- ✅ Создан автономный бинарный файл FSA-AstraInstall (PyInstaller)
- ✅ Настроена удалённая сборка через Docker на Windows Server (WSL2)
- ✅ Добавлена поддержка Portainer для мониторинга сборки
- ✅ Оптимизирована совместимость с GLIBC 2.28 (Astra 1.7+)
...
Бинарный файл собирается через Docker на удалённом сервере (Windows + WSL2):

```bash
# На macOS с настроенным Docker context
python3 build_executables.py
```

**Процесс сборки:**
1. Объединение Python-файлов в единый `FSA-AstraInstall.py`
2. Сборка в Docker-контейнере (Debian Buster, GLIBC 2.28)
3. PyInstaller создаёт автономный бинарник
4. Результат: `FSA-AstraInstall` (11.8 MB)

**Требования для сборки:**
- Docker (локальный или удалённый через SSH)
- Python 3.7+
- PyInstaller
