# 📋 ДЕТАЛЬНЫЙ ПЛАН РЕФАКТОРИНГА: МИГРАЦИЯ НА АРХИТЕКТУРУ COMPONENT HANDLERS
# Версия проекта: V2.4.94 (2025.10.31)
# Компания: ООО "НПА Вира-Реалтайм"
# Статус: ПЛАН РЕАЛИЗАЦИИ

---

## 🎯 ЦЕЛИ И ЗАДАЧИ РЕФАКТОРИНГА

### Основные цели:
1. **Упрощение кода** - удаление дублирующей логики (~2037 строк)
2. **Расширяемость** - добавление новых типов компонентов без изменения `UniversalInstaller`
3. **Унификация** - единая точка входа для всех компонентов через handlers
4. **Интеграция** - использование всей инфраструктуры (логирование, прогресс-бары, процессы)
5. **Безопасность** - миграция без ломки существующего функционала

### Критерии успеха:
- ✅ Существующий функционал продолжает работать
- ✅ Новые компоненты устанавливаются/удаляются через handlers
- ✅ Логирование работает через все каналы
- ✅ Прогресс-бары отображаются корректно
- ✅ Код стал проще и понятнее

---

## 📊 ТЕКУЩЕЕ СОСТОЯНИЕ

### Существующие классы:

#### 1. Классы, которые БУДУТ УДАЛЕНЫ:
- **`WineInstaller`** (строки 2265-3815, ~1550 строк)
  - Методы: `install_wine_packages()`, `setup_wine_environment()`, `_ensure_wineprefix_architecture()`, `install_winetricks_components()`, `install_astra_ide()`, `create_start_script()`, `create_desktop_shortcut()`
  
- **`WineUninstaller`** (строки 4272-4759, ~487 строк)
  - Методы: `remove_wine_packages()`, `remove_wineprefix()`, `remove_ide()`, `remove_all_wine_data()`

#### 2. Классы, которые ОСТАНУТСЯ:
- **`WinetricksManager`** - используется handlers
- **`MinimalWinetricks`** - используется WinetricksManager
- **`UniversalInstaller`** - будет модифицирован
- **`WineComponentsChecker`** - проверка компонентов (остается)
- **`UniversalProcessRunner`** - используется handlers
- **`DualStreamLogger`** - используется handlers
- **`UniversalProgressManager`** - используется handlers

#### 3. Классы, которые БУДУТ ДОБАВЛЕНЫ:
- **`ComponentHandler`** - базовый абстрактный класс
- **`WinePackageHandler`** - обработчик Wine пакетов
- **`WineEnvironmentHandler`** - обработчик Wine окружения
- **`WinetricksHandler`** - обработчик winetricks компонентов
- **`SystemConfigHandler`** - обработчик системных настроек
- **`ApplicationHandler`** - обработчик приложений (Astra.IDE)

---

## 🏗️ ЦЕЛЕВАЯ АРХИТЕКТУРА

### Структура handlers:

```
ComponentHandler (абстрактный базовый класс)
├── WinePackageHandler (wine_packages)
│   ├── install() - установка через apt
│   ├── uninstall() - удаление через apt-get purge
│   └── check_status() - проверка через check_paths
│
├── WineEnvironmentHandler (wine_environment)
│   ├── install() - инициализация WINEPREFIX + копирование кэша
│   ├── uninstall() - удаление WINEPREFIX
│   └── check_status() - проверка существования WINEPREFIX
│
├── WinetricksHandler (winetricks)
│   ├── install() - установка через WinetricksManager
│   ├── uninstall() - удаление winetricks компонентов
│   └── check_status() - проверка через check_paths
│
├── SystemConfigHandler (system_config)
│   ├── install() - настройка ptrace_scope и т.д.
│   ├── uninstall() - откат системных настроек
│   └── check_status() - проверка текущих настроек
│
└── ApplicationHandler (application)
    ├── install() - установка .exe + создание скриптов/ярлыков
    ├── uninstall() - удаление приложения + скриптов/ярлыков
    └── check_status() - проверка через check_paths
```

### Интеграция с инфраструктурой:

```
ComponentHandler
├── universal_runner (UniversalProcessRunner)
│   ├── Запуск процессов через run_process()
│   ├── Логирование через log_info/log_error
│   └── Интеграция с GUI через gui_callback
│
├── progress_manager (UniversalProgressManager)
│   ├── Обновление прогресса через update_progress()
│   └── Отображение в GUI прогресс-баров
│
├── dual_logger (DualStreamLogger)
│   ├── RAW поток через write_raw()
│   └── ANALYSIS поток через write_analysis()
│
└── callback (функция для GUI)
    └── Обновление статуса в GUI
```

---

## 📝 ПОШАГОВЫЙ ПЛАН РЕАЛИЗАЦИИ

### ЭТАП 0: ПОДГОТОВКА (Приоритет: КРИТИЧЕСКИЙ)

#### Шаг 0.1: Создание документации плана
- [x] Создан план рефакторинга (`REFACTORING_PLAN_COMPONENT_HANDLERS.md`)
- [ ] Документ сохранен в корне проекта

#### Шаг 0.2: Бэкап текущего состояния
- [ ] Создать git ветку: `git checkout -b refactoring/component-handlers`
- [ ] Сделать коммит текущего состояния: `git commit -am "Текущее состояние перед рефакторингом"`
- [ ] Проверить, что все тесты проходят

#### Шаг 0.3: Анализ зависимостей
- [ ] Найти все места использования `WineInstaller`:
  ```bash
  grep -n "WineInstaller" astra_automation.py
  ```
- [ ] Найти все места использования `WineUninstaller`:
  ```bash
  grep -n "WineUninstaller" astra_automation.py
  ```
- [ ] Составить список зависимостей для миграции

---

### ЭТАП 1: СОЗДАНИЕ БАЗОВОГО КЛАССА (Приоритет: ВЫСОКИЙ)

#### Шаг 1.1: Создание абстрактного класса ComponentHandler
**Местоположение:** Вставить после строки 232 (после `COMPONENTS_CONFIG`)

**Что делать:**
1. Добавить импорт: `from abc import ABC, abstractmethod` (если нет в начале файла)
2. Создать класс `ComponentHandler` с базовыми методами:
   - `__init__()` - инициализация с инфраструктурой
   - `_log()` - универсальное логирование
   - `_update_progress()` - обновление прогресса
   - `_run_process()` - запуск процессов
   - Абстрактные методы: `install()`, `uninstall()`, `check_status()`, `get_category()`

**Код для вставки:**

```python
# ============================================================================
# БАЗОВЫЙ КЛАСС ДЛЯ ОБРАБОТЧИКОВ КОМПОНЕНТОВ
# ============================================================================
from abc import ABC, abstractmethod

class ComponentHandler(ABC):
    """Базовый класс для обработчиков компонентов с интеграцией всех систем"""
    
    def __init__(self, 
                 astrapack_dir=None,
                 logger=None,
                 callback=None,
                 universal_runner=None,
                 progress_manager=None,
                 dual_logger=None):
        """
        Инициализация обработчика компонентов
        
        Args:
            astrapack_dir: Путь к директории AstraPack
            logger: Экземпляр Logger для записи в файл
            callback: Функция для обновления статуса в GUI
            universal_runner: UniversalProcessRunner для запуска процессов
            progress_manager: UniversalProgressManager для прогресса
            dual_logger: DualStreamLogger для двойных потоков логирования
        """
        self.astrapack_dir = astrapack_dir
        self.logger = logger
        self.callback = callback
        self.universal_runner = universal_runner or get_global_universal_runner()
        self.progress_manager = progress_manager
        self.dual_logger = dual_logger
        
        # Определяем домашнюю директорию РЕАЛЬНОГО пользователя
        real_user = os.environ.get('SUDO_USER')
        if real_user and real_user != 'root':
            import pwd
            self.home = pwd.getpwnam(real_user).pw_dir
        else:
            self.home = os.path.expanduser("~")
        
        self.wineprefix = os.path.join(self.home, ".wine-astraregul")
    
    def _log(self, message, level="INFO"):
        """Универсальное логирование через все системы"""
        # Логирование через UniversalProcessRunner
        if self.universal_runner:
            if level == "ERROR":
                self.universal_runner.log_error(message)
            elif level == "WARNING":
                self.universal_runner.log_warning(message)
            else:
                self.universal_runner.log_info(message)
        
        # Логирование через DualStreamLogger (если есть)
        if self.dual_logger:
            if level == "ERROR":
                self.dual_logger.write_analysis(f"[ERROR] {message}")
            elif level == "WARNING":
                self.dual_logger.write_analysis(f"[WARNING] {message}")
            else:
                self.dual_logger.write_analysis(f"[INFO] {message}")
        
        # Callback в GUI
        if self.callback:
            self.callback(message)
    
    def _update_progress(self, stage_name, stage_progress, global_progress, details=""):
        """Обновление прогресса через UniversalProgressManager"""
        if self.progress_manager:
            self.progress_manager.update_progress(
                process_type=self.get_category(),
                stage_name=stage_name,
                stage_progress=stage_progress,
                global_progress=global_progress,
                details=details
            )
    
    def _run_process(self, command, process_type="general", channels=["file", "terminal"]):
        """Запуск процесса через UniversalProcessRunner"""
        if self.universal_runner:
            return self.universal_runner.run_process(
                command=command,
                process_type=process_type,
                channels=channels
            )
        return -1
    
    @abstractmethod
    def install(self, component_id: str, config: dict) -> bool:
        """Установка компонента"""
        pass
    
    @abstractmethod
    def uninstall(self, component_id: str, config: dict) -> bool:
        """Удаление компонента"""
        pass
    
    @abstractmethod
    def check_status(self, component_id: str, config: dict) -> bool:
        """Проверка статуса компонента"""
        pass
    
    @abstractmethod
    def get_category(self) -> str:
        """Возвращает категорию компонентов"""
        pass
```

**Чеклист:**
- [ ] Код вставлен после `COMPONENTS_CONFIG`
- [ ] Импорт `ABC, abstractmethod` добавлен в начало файла (если нет)
- [ ] Код проверен на синтаксис: `python3 -m py_compile astra_automation.py`

---

### ЭТАП 2: СОЗДАНИЕ HANDLERS (Приоритет: ВЫСОКИЙ)

#### Шаг 2.1: Создание WinePackageHandler
**Местоположение:** Вставить после класса `ComponentHandler` (после шага 1.1)

**Что делать:**
1. Изучить метод `_install_package_manager()` из `UniversalInstaller` (строки 5063-5099)
2. Изучить метод `_uninstall_package_manager()` из `UniversalInstaller` (строки 5157-5233)
3. Создать класс `WinePackageHandler` с методами:
   - `install()` - установка через apt
   - `uninstall()` - удаление через apt-get purge
   - `check_status()` - проверка через check_paths
   - `get_category()` - возвращает 'wine_packages'

**Код для вставки:**

```python
# ============================================================================
# ОБРАБОТЧИК WINE ПАКЕТОВ
# ============================================================================
class WinePackageHandler(ComponentHandler):
    """Обработчик Wine пакетов (wine_astraregul, wine_9)"""
    
    def get_category(self) -> str:
        return 'wine_packages'
    
    def install(self, component_id: str, config: dict) -> bool:
        """Установка Wine пакета через apt"""
        self._log(f"Установка компонента: {config['name']}")
        
        # Обновляем прогресс
        self._update_progress(
            stage_name=f"Установка {config['name']}",
            stage_progress=0,
            global_progress=0,
            details=f"Подготовка к установке {component_id}"
        )
        
        # Определяем имя .deb файла из component_id
        deb_files = {
            'wine_astraregul': 'wine-astraregul_10.0-rc6-3_amd64.deb',
            'wine_9': 'wine_9.0-1_amd64.deb'
        }
        
        if component_id not in deb_files:
            self._log(f"ОШИБКА: Неизвестный пакет: {component_id}", "ERROR")
            return False
        
        deb_name = deb_files[component_id]
        package_path = os.path.join(self.astrapack_dir, deb_name)
        
        if not os.path.exists(package_path):
            self._log(f"ОШИБКА: Файл пакета не найден: {package_path}", "ERROR")
            return False
        
        # Используем UniversalProcessRunner вместо subprocess
        self._update_progress(
            stage_name=f"Установка {config['name']}",
            stage_progress=50,
            global_progress=50,
            details="Установка через apt..."
        )
        
        return_code = self._run_process(
            ['apt', 'install', '-y', package_path],
            process_type="install",
            channels=["file", "terminal", "gui"]
        )
        
        if return_code == 0:
            self._log(f"Пакет {config['name']} установлен успешно")
            self._update_progress(
                stage_name=f"Установка {config['name']}",
                stage_progress=100,
                global_progress=100,
                details=f"{config['name']} установлен успешно"
            )
            return True
        else:
            self._log(f"ОШИБКА установки пакета {config['name']} (код: {return_code})", "ERROR")
            return False
    
    def uninstall(self, component_id: str, config: dict) -> bool:
        """Удаление Wine пакета через apt-get purge"""
        self._log(f"Удаление компонента: {config['name']}")
        
        package_names = {
            'wine_astraregul': 'wine-astraregul',
            'wine_9': 'wine-9.0'
        }
        
        if component_id not in package_names:
            self._log(f"ОШИБКА: Неизвестный пакет для удаления: {component_id}", "ERROR")
            return False
        
        package_name = package_names[component_id]
        
        # Останавливаем Wine процессы перед удалением
        self._run_process(['pkill', '-9', 'wine'], process_type="remove")
        self._run_process(['pkill', '-9', 'wineserver'], process_type="remove")
        self._run_process(['pkill', '-9', 'wineboot'], process_type="remove")
        
        import time
        time.sleep(1)
        
        # Удаляем пакет через apt-get purge
        return_code = self._run_process(
            ['apt-get', 'purge', '-y', package_name],
            process_type="remove",
            channels=["file", "terminal"]
        )
        
        if return_code == 0:
            self._log(f"Пакет {config['name']} удален успешно")
            # Удаляем директории Wine вручную
            wine_dirs = {
                'wine_astraregul': '/opt/wine-astraregul',
                'wine_9': '/opt/wine-9.0'
            }
            if component_id in wine_dirs:
                wine_dir = wine_dirs[component_id]
                if os.path.exists(wine_dir):
                    import shutil
                    shutil.rmtree(wine_dir)
                    self._log(f"  Удалена директория: {wine_dir}")
            return True
        else:
            self._log(f"ОШИБКА удаления пакета {config['name']}", "ERROR")
            return False
    
    def check_status(self, component_id: str, config: dict) -> bool:
        """Проверка статуса Wine пакета"""
        check_paths = config.get('check_paths', [])
        for path in check_paths:
            if os.path.exists(path):
                return True
        return False
```

**Чеклист:**
- [ ] Класс создан и наследуется от `ComponentHandler`
- [ ] Метод `install()` работает для `wine_astraregul` и `wine_9`
- [ ] Метод `uninstall()` останавливает процессы и удаляет пакеты
- [ ] Метод `check_status()` проверяет существование файлов из `check_paths`
- [ ] Используется `_run_process()` вместо прямого `subprocess.run()`
- [ ] Используется `_log()` для логирования
- [ ] Используется `_update_progress()` для прогресса

#### Шаг 2.2: Создание WineEnvironmentHandler
**Местоположение:** После `WinePackageHandler`

**Что делать:**
1. Изучить метод `setup_wine_environment()` из `WineInstaller` (строки 3094-3168)
2. Изучить метод `_ensure_wineprefix_architecture()` из `WineInstaller` (строки 2483-2546)
3. Создать класс `WineEnvironmentHandler` с методами:
   - `install()` - инициализация WINEPREFIX + копирование кэша
   - `uninstall()` - удаление WINEPREFIX
   - `check_status()` - проверка существования WINEPREFIX
   - `get_category()` - возвращает 'wine_environment'

**Чеклист:**
- [ ] Класс создан
- [ ] Метод `install()` копирует `wine-gecko` и `winetricks-cache`
- [ ] Метод `install()` инициализирует WINEPREFIX с архитектурой win64
- [ ] Метод `uninstall()` удаляет директорию WINEPREFIX
- [ ] Метод `check_status()` проверяет существование `~/.wine-astraregul`
- [ ] Используется инфраструктура для логирования и прогресса

#### Шаг 2.3: Создание WinetricksHandler
**Местоположение:** После `WineEnvironmentHandler`

**Что делать:**
1. Изучить метод `install_winetricks_components()` из `WineInstaller` (строки 3325-3384)
2. Изучить класс `WinetricksManager` (строки 3820-3945)
3. Создать класс `WinetricksHandler` с методами:
   - `install()` - установка через WinetricksManager
   - `uninstall()` - удаление winetricks компонентов (через удаление WINEPREFIX)
   - `check_status()` - проверка через check_paths
   - `get_category()` - возвращает 'winetricks'

**Чеклист:**
- [ ] Класс создан
- [ ] Инициализируется `WinetricksManager` в конструкторе
- [ ] Метод `install()` вызывает `WinetricksManager.install_wine_packages()`
- [ ] Метод `install()` передает правильный `wineprefix`
- [ ] Метод `install()` использует callback для обновления статуса
- [ ] Метод `check_status()` проверяет файлы из `check_paths` в WINEPREFIX
- [ ] Поддерживается флаг `use_minimal_winetricks`

#### Шаг 2.4: Создание SystemConfigHandler
**Местоположение:** После `WinetricksHandler`

**Что делать:**
1. Изучить метод `_install_system_config()` из `UniversalInstaller` (строки 5101-5129)
2. Создать класс `SystemConfigHandler` с методами:
   - `install()` - настройка ptrace_scope
   - `uninstall()` - откат системных настроек
   - `check_status()` - проверка текущих настроек
   - `get_category()` - возвращает 'system_config'

**Чеклист:**
- [ ] Класс создан
- [ ] Метод `install()` настраивает `ptrace_scope=0` через `sysctl`
- [ ] Метод `uninstall()` возвращает `ptrace_scope` к исходному значению
- [ ] Метод `check_status()` проверяет значение в `/proc/sys/kernel/yama/ptrace_scope`

#### Шаг 2.5: Создание ApplicationHandler
**Местоположение:** После `SystemConfigHandler`

**Что делать:**
1. Изучить метод `install_astra_ide()` из `WineInstaller` (строки 3581-3763)
2. Изучить метод `create_start_script()` из `WineInstaller` (строки 3513-3579)
3. Создать класс `ApplicationHandler` с методами:
   - `install()` - установка .exe + создание скриптов/ярлыков
   - `uninstall()` - удаление приложения + скриптов/ярлыков
   - `check_status()` - проверка через check_paths
   - `get_category()` - возвращает 'application'

**Чеклист:**
- [ ] Класс создан
- [ ] Метод `install()` устанавливает .exe через Wine
- [ ] Метод `install()` создает скрипт запуска `~/start-astraide.sh`
- [ ] Метод `install()` создает ярлык `~/Desktop/AstraRegul.desktop`
- [ ] Метод `uninstall()` удаляет приложение, скрипт и ярлык
- [ ] Метод `check_status()` проверяет существование .exe, скрипта и ярлыка
- [ ] Устанавливается правильный владелец файлов при sudo

---

### ЭТАП 3: ИНТЕГРАЦИЯ HANDLERS В UNIVERSALINSTALLER (Приоритет: КРИТИЧЕСКИЙ)

#### Шаг 3.1: Добавление флага use_handlers
**Местоположение:** Метод `__init__()` класса `UniversalInstaller` (строка 4768)

**Изменения:**
```python
def __init__(self, logger=None, callback=None, 
             use_handlers=False,  # НОВЫЙ ПАРАМЕТР
             use_minimal_winetricks=True,  # НОВЫЙ ПАРАМЕТР
             universal_runner=None,  # НОВЫЙ ПАРАМЕТР
             progress_manager=None,  # НОВЫЙ ПАРАМЕТР
             dual_logger=None):  # НОВЫЙ ПАРАМЕТР
    # ... существующий код ...
    self.use_handlers = use_handlers
    self.use_minimal_winetricks = use_minimal_winetricks
    self.universal_runner = universal_runner or get_global_universal_runner()
    self.progress_manager = progress_manager
    self.dual_logger = dual_logger
    
    # Реестр обработчиков компонентов
    self.handlers = {}
    if self.use_handlers:
        self._register_handlers()
```

**Чеклист:**
- [ ] Параметр `use_handlers` добавлен
- [ ] Параметр `use_minimal_winetricks` добавлен
- [ ] Параметры инфраструктуры добавлены
- [ ] Сохранены в `self.use_handlers`, `self.use_minimal_winetricks` и т.д.

#### Шаг 3.2: Регистрация handlers
**Местоположение:** После `__init__()` класса `UniversalInstaller`

**Код для вставки:**

```python
def _register_handlers(self):
    """Регистрация всех обработчиков компонентов"""
    # Общие параметры для всех handlers
    common_params = {
        'astrapack_dir': self.astrapack_dir,
        'logger': self.logger,
        'callback': self.callback,
        'universal_runner': self.universal_runner,
        'progress_manager': self.progress_manager,
        'dual_logger': self.dual_logger
    }
    
    # Регистрируем handlers
    self.handlers['wine_packages'] = WinePackageHandler(**common_params)
    self.handlers['wine_environment'] = WineEnvironmentHandler(**common_params)
    self.handlers['winetricks'] = WinetricksHandler(
        use_minimal=self.use_minimal_winetricks,
        **common_params
    )
    self.handlers['system_config'] = SystemConfigHandler(**common_params)
    self.handlers['application'] = ApplicationHandler(**common_params)
```

**Чеклист:**
- [ ] Метод `_register_handlers()` создан
- [ ] Вызывается в `__init__()` если `use_handlers=True`
- [ ] Все handlers зарегистрированы
- [ ] Инфраструктура передается во все handlers

#### Шаг 3.3: Модификация метода install_component()
**Местоположение:** Метод `install_component()` класса `UniversalInstaller` (строка 4917)

**Изменения:**
```python
def install_component(self, component_id):
    """Установка одного компонента"""
    if component_id not in COMPONENTS_CONFIG:
        self._log("Ошибка: компонент '%s' не найден в конфигурации" % component_id, "ERROR")
        return False
    
    config = COMPONENTS_CONFIG[component_id]
    
    # НОВАЯ ЛОГИКА: Используем handlers если включен флаг
    if self.use_handlers:
        category = config.get('category')
        if category in self.handlers:
            handler = self.handlers[category]
            return handler.install(component_id, config)
        else:
            self._log(f"Неизвестная категория: {category}", "ERROR")
            return False
    
    # СТАРАЯ ЛОГИКА: Используем install_method (оставляем для совместимости)
    install_method = config['install_method']
    # ... существующий код ...
```

**Чеклист:**
- [ ] Проверка `use_handlers` добавлена
- [ ] Новая логика использует handlers
- [ ] Старая логика сохранена и работает
- [ ] Обработка ошибок работает для обоих путей

#### Шаг 3.4: Модификация метода uninstall_component()
**Местоположение:** Метод `uninstall_component()` класса `UniversalInstaller` (строка 4958)

**Что делать:** Аналогично `install_component()` - добавить проверку `use_handlers`

**Чеклист:**
- [ ] Аналогично шагу 3.3

---

### ЭТАП 4: ИНТЕГРАЦИЯ В GUI (Приоритет: ВЫСОКИЙ)

#### Шаг 4.1: Обновление создания UniversalInstaller в GUI
**Местоположение:** Метод `__init__()` класса `AutomationGUI` (строка 5828)

**Изменения:**
```python
self.universal_installer = UniversalInstaller(
    callback=self._component_status_callback,
    use_handlers=True,  # Включаем новую архитектуру
    use_minimal_winetricks=self.use_minimal_winetricks.get(),
    universal_runner=self.universal_runner,
    progress_manager=self.progress_manager,
    dual_logger=_global_dual_logger if '_global_dual_logger' in globals() else None
)
```

**Чеклист:**
- [ ] Параметр `use_handlers=True` передан
- [ ] Параметр `use_minimal_winetricks` передан из GUI
- [ ] Инфраструктура передана корректно

---

### ЭТАП 5: ТЕСТИРОВАНИЕ (Приоритет: КРИТИЧЕСКИЙ)

#### Шаг 5.1: Тестирование каждого handler по отдельности

**WinePackageHandler:**
- [ ] Тест установки `wine_astraregul` через handler
- [ ] Тест установки `wine_9` через handler
- [ ] Тест удаления `wine_astraregul` через handler
- [ ] Тест проверки статуса компонентов
- [ ] Тест логирования (проверить файлы логов)
- [ ] Тест прогресс-баров (проверить обновление в GUI)

**WineEnvironmentHandler:**
- [ ] Тест инициализации WINEPREFIX через handler
- [ ] Тест копирования wine-gecko
- [ ] Тест копирования winetricks-cache
- [ ] Тест удаления WINEPREFIX
- [ ] Тест проверки статуса

**WinetricksHandler:**
- [ ] Тест установки winetricks компонентов (dotnet48, vcrun2013 и т.д.)
- [ ] Тест проверки статуса компонентов
- [ ] Тест с флагом `use_minimal_winetricks=True`
- [ ] Тест с флагом `use_minimal_winetricks=False`

**SystemConfigHandler:**
- [ ] Тест настройки `ptrace_scope`
- [ ] Тест проверки статуса
- [ ] Тест отката настроек

**ApplicationHandler:**
- [ ] Тест установки Astra.IDE
- [ ] Тест создания скрипта запуска
- [ ] Тест создания ярлыка
- [ ] Тест удаления всех компонентов
- [ ] Тест проверки статуса

#### Шаг 5.2: Интеграционное тестирование

**Полный цикл установки:**
- [ ] Установка всех компонентов через `UniversalInstaller.install_components()`
- [ ] Проверка, что зависимости разрешаются правильно
- [ ] Проверка, что все логи записываются
- [ ] Проверка, что прогресс-бары обновляются
- [ ] Проверка, что компоненты устанавливаются в правильном порядке

**Полный цикл удаления:**
- [ ] Удаление всех компонентов через `UniversalInstaller.uninstall_components()`
- [ ] Проверка, что дочерние компоненты удаляются
- [ ] Проверка, что компоненты удаляются в правильном порядке

**Сравнение со старой логикой:**
- [ ] Установка через handlers vs через старую логику - результат одинаковый
- [ ] Логи идентичны
- [ ] Прогресс-бары работают одинаково

#### Шаг 5.3: Регрессионное тестирование

**Проверка, что старый функционал работает:**
- [ ] Установка с `use_handlers=False` работает (старая логика)
- [ ] Все существующие тесты проходят
- [ ] GUI отображается корректно
- [ ] Все кнопки работают

---

### ЭТАП 6: МИГРАЦИЯ НА НОВУЮ АРХИТЕКТУРУ (Приоритет: СРЕДНИЙ)

#### Шаг 6.1: Постепенное включение handlers

**План миграции:**
1. **Фаза 1:** Включить handlers только для `wine_packages`
   - [ ] Установить `use_handlers=True` только для категории `wine_packages`
   - [ ] Протестировать установку/удаление Wine пакетов
   - [ ] Если все ОК - перейти к фазе 2

2. **Фаза 2:** Добавить `wine_environment`
   - [ ] Включить handlers для `wine_packages` + `wine_environment`
   - [ ] Протестировать полный цикл Wine
   - [ ] Если все ОК - перейти к фазе 3

3. **Фаза 3:** Добавить `winetricks`
   - [ ] Включить handlers для всех категорий Wine
   - [ ] Протестировать установку winetricks компонентов
   - [ ] Если все ОК - перейти к фазе 4

4. **Фаза 4:** Добавить `system_config` и `application`
   - [ ] Включить handlers для всех категорий
   - [ ] Протестировать полный цикл установки всех компонентов
   - [ ] Если все ОК - перейти к этапу 7

#### Шаг 6.2: Мониторинг и откат

**Процедура отката:**
- [ ] Если что-то пошло не так - установить `use_handlers=False`
- [ ] Старая логика должна работать как раньше
- [ ] Зафиксировать проблему в логах
- [ ] Исправить проблему в handlers
- [ ] Повторить тестирование

---

### ЭТАП 7: УДАЛЕНИЕ СТАРОГО КОДА (Приоритет: НИЗКИЙ)

#### Шаг 7.1: Удаление WineInstaller

**Условия:**
- [ ] Все handlers протестированы и работают
- [ ] `use_handlers=True` работает для всех категорий
- [ ] Никто не использует `WineInstaller` напрямую

**Что делать:**
1. Найти все использования `WineInstaller`
2. Удалить или закомментировать класс `WineInstaller` (строки 2265-3815)
3. Протестировать, что ничего не сломалось

**Чеклист:**
- [ ] Класс `WineInstaller` удален или закомментирован
- [ ] Все ссылки на него удалены
- [ ] Тесты проходят
- [ ] GUI работает

#### Шаг 7.2: Удаление WineUninstaller

**Условия:**
- [ ] Все handlers поддерживают `uninstall()`
- [ ] `use_handlers=True` работает для удаления

**Что делать:**
1. Найти все использования `WineUninstaller`
2. Удалить или закомментировать класс `WineUninstaller` (строки 4272-4759)
3. Протестировать удаление компонентов

**Чеклист:**
- [ ] Класс `WineUninstaller` удален или закомментирован
- [ ] Все ссылки на него удалены
- [ ] Тесты удаления проходят

#### Шаг 7.3: Очистка UniversalInstaller

**Что делать:**
1. Удалить старые методы установки из `UniversalInstaller`
2. Удалить старые методы удаления
3. Упростить методы `install_component()` и `uninstall_component()`

**Чеклист:**
- [ ] Все старые методы удалены
- [ ] `install_component()` и `uninstall_component()` упрощены
- [ ] Код стал короче и понятнее
- [ ] Все тесты проходят

#### Шаг 7.4: Удаление флага use_handlers

**Что делать:**
1. Удалить параметр `use_handlers` из `__init__()`
2. Удалить проверку `if self.use_handlers:` везде
3. Удалить старую логику полностью

**Чеклист:**
- [ ] Флаг `use_handlers` удален
- [ ] Код упрощен
- [ ] Тесты проходят

---

### ЭТАП 8: ФИНАЛИЗАЦИЯ (Приоритет: СРЕДНИЙ)

#### Шаг 8.1: Обновление документации
- [ ] Обновить `README.md` с описанием новой архитектуры
- [ ] Обновить `WINE_INSTALL_GUIDE.md`
- [ ] Добавить комментарии в код handlers
- [ ] Обновить примеры использования

#### Шаг 8.2: Оптимизация и рефакторинг
- [ ] Проверить, нет ли дублирования кода между handlers
- [ ] Вынести общую логику в базовый класс `ComponentHandler`
- [ ] Оптимизировать методы handlers
- [ ] Улучшить обработку ошибок

#### Шаг 8.3: Финальное тестирование
- [ ] Полный цикл установки всех компонентов
- [ ] Полный цикл удаления всех компонентов
- [ ] Проверка всех граничных случаев
- [ ] Проверка производительности

#### Шаг 8.4: Коммит и слияние
- [ ] Создать коммит с описанием изменений
- [ ] Слить ветку `refactoring/component-handlers` в основную ветку
- [ ] Создать тег версии
- [ ] Обновить версию в коде

---

## 🔍 КРИТЕРИИ УСПЕХА

### Функциональные критерии:
- ✅ Все существующие компоненты устанавливаются/удаляются корректно
- ✅ Новые компоненты можно добавлять только через handlers
- ✅ Логирование работает через все каналы (файл, терминал, GUI)
- ✅ Прогресс-бары отображаются для всех операций
- ✅ Зависимости разрешаются автоматически

### Технические критерии:
- ✅ Код стал короче (удалено ~2037 строк дублирующего кода)
- ✅ Код стал понятнее (логика инкапсулирована в handlers)
- ✅ Код стал расширяемее (новый handler = новый тип компонентов)
- ✅ Нет дублирования логики между handlers

### Качественные критерии:
- ✅ Все тесты проходят
- ✅ Нет регрессий в функционале
- ✅ Код задокументирован
- ✅ Комментарии понятны новому разработчику

---

## ⚠️ РИСКИ И СПОСОБЫ ИХ МИНИМИЗАЦИИ

### Риск 1: Поломка существующего функционала
**Минимизация:**
- Использовать флаг `use_handlers` для постепенного переключения
- Сохранить старую логику до полного тестирования
- Регрессионное тестирование на каждом этапе

### Риск 2: Потеря логирования или прогресса
**Минимизация:**
- Каждый handler использует инфраструктуру через базовый класс
- Тестировать логирование и прогресс для каждого handler
- Проверять все каналы вывода

### Риск 3: Ошибки в handlers
**Минимизация:**
- Пошаговое тестирование каждого handler
- Unit-тесты для критических методов
- Интеграционные тесты для полного цикла

### Риск 4: Проблемы с миграцией
**Минимизация:**
- Поэтапная миграция (один handler за раз)
- Возможность отката через `use_handlers=False`
- Детальное логирование всех операций

---

## 📚 СПРАВОЧНАЯ ИНФОРМАЦИЯ

### Связь компонентов:
```
COMPONENTS_CONFIG['category']
    ↓
UniversalInstaller.handlers['category']
    ↓
ComponentHandler.install/uninstall
    ↓
UniversalProcessRunner / DualStreamLogger / UniversalProgressManager
```

### Порядок установки компонентов:
1. `wine_packages` (Wine пакеты)
2. `system_config` (ptrace_scope)
3. `wine_environment` (WINEPREFIX)
4. `winetricks` (winetricks компоненты)
5. `application` (Astra.IDE + скрипты + ярлыки)

### Порядок удаления компонентов:
1. `application` (приложение + скрипты + ярлыки)
2. `winetricks` (winetricks компоненты)
3. `wine_environment` (WINEPREFIX)
4. `system_config` (откат настроек)
5. `wine_packages` (Wine пакеты)

---

## 📝 ЗАМЕТКИ ДЛЯ РАЗРАБОТЧИКА

### Важные моменты:
1. **Всегда тестируй после каждого шага** - не переходи к следующему этапу без проверки
2. **Используй git** - коммить после каждого успешного шага
3. **Документируй проблемы** - если что-то не работает, зафиксируй в комментариях
4. **Не торопись** - лучше медленно, но правильно, чем быстро с ошибками

### Команды для проверки:
```bash
# Проверка синтаксиса
python3 -m py_compile astra_automation.py

# Поиск использования класса
grep -n "WineInstaller" astra_automation.py

# Запуск тестов (если есть)
./test-environment/test_all_components.sh
```

---

## ✅ ЧЕКЛИСТ ФИНАЛЬНОЙ ПРОВЕРКИ

Перед завершением рефакторинга проверить:
- [ ] Все handlers реализованы и работают
- [ ] Все тесты проходят
- [ ] Старый код удален
- [ ] Документация обновлена
- [ ] Нет дублирования кода
- [ ] Логирование работает
- [ ] Прогресс-бары работают
- [ ] GUI работает корректно
- [ ] Коммит создан с описанием изменений
- [ ] Ветка слита в основную

---

**КОНЕЦ ПЛАНА РЕФАКТОРИНГА**

Дата создания плана: 2025-10-31
Версия плана: 1.0
Статус: ГОТОВ К РЕАЛИЗАЦИИ

